...
Copyright (C) 2017, perlatus <perlatus@zoomable_image.email.vczf.io>
...
class _ZoomableImageState extends State<ZoomableImage> {
  ...
  void _centerAndScaleImage() {
    ...
    Size fitted = new Size(_imageSize.width * _scale, _imageSize.height * _scale);

    Offset delta = _canvasSize - fitted;
    _offset = delta / 2.0; // Centers the image
  }

  Function() _handleDoubleTap(BuildContext context) {
    return () {
      ...
      Offset center = context.size.center(Offset.zero);
  ...
  void _handleScaleUpdate(ScaleUpdateDetails d) {
    ...
    if (newScale > widget.maxScale || newScale < widget.minScale) return;

    // Ensure that item under the focal point stays in the same place despite zooming
    Offset normalizedOffset = (_startingFocalPoint - _previousOffset) / _previousScale;
    Offset wishedOffset = d.focalPoint - normalizedOffset * newScale;

    double obtainedX = wishedOffset.dx;
    double obtainedY = wishedOffset.dy;

    if (obtainedX.abs() > (_imageSize.width * newScale) - _canvasSize.width) {
      obtainedX = _canvasSize.width - (_imageSize.width * newScale);
    }

    if (obtainedY.abs() > (_imageSize.height * newScale) - _canvasSize.height) {
      obtainedY = _canvasSize.height - (_imageSize.height * newScale);
    }

    if (wishedOffset.dx > 0) {
      obtainedX = 0;
    }

    if (wishedOffset.dy > 0) {
      obtainedY = 0;
    }

    Offset obtainedOffset = Offset(obtainedX, obtainedY);

    setState(() {
      _scale = newScale;
      _offset = obtainedOffset;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_image == null) return widget.placeholder ?? Center(child: CircularProgressIndicator());
